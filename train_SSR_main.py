...
def train():
    ...
    print(f'Experiment GPU is {config["experiment"]["gpu"]}.')
    ...
    if args.dataset_type == "replica":
        ...
        if args.super_resolution:
            print(
                f"Super Resolution Mode! Dense Label Flag is {args.dense_sr}, SR Factor is {args.sr_factor}"
            )
            ...
            print(
                f"Label Propagation Mode! Partial labelling percentage is: {args.partial_perc} "
            )
            ...
            print(
                f"Chair Label Flipping for Region-wise Denoising, Flip ratio is {args.region_noise_ratio}, Uniform Sampling is {args.uniform_flip}"
            )
        ...
        print(
            f'Replica_nyu_cnn mode using labels from trained CNNs: {config["experiment"]["nyu_mode"]}'
        )
    ...
    ##########################
    print('Begin')
    for global_step, _ in enumerate(trange(global_step, N_iters)):
        time0 = time.time()
